...
def driver_single_image(query_img_path, rootdir, destination, dataset_name, np_seed, tf_seed):
    ...
    start = time.time()
    subdir1 = query_img_path
    # print(subdir1.split("\\"))
    print(dataset_name)
    query1_name  = subdir1.split("\\")[-3]
    ...
    result_df = result_df.sort_values(by=["siamese_distance"])
    print(result_df)
    result_df.to_csv(destination + "\\" + query1_name +".csv")
    
    APlist_3.append(calculateAvgPrecision(result_df, 3))
    RRlist_3.append(calculateReciprocalRank(result_df, 3))
    
    APlist_5.append(calculateAvgPrecision(result_df, 5))
    RRlist_5.append(calculateReciprocalRank(result_df, 5))
    
    APlist_7.append(calculateAvgPrecision(result_df, 7))
    RRlist_7.append(calculateReciprocalRank(result_df, 7))
    
        # print(APlist, RRlist)
    end = time.time()
    metric_result["query image"].append(query1_name)
    metric_result["k"].append("3, 5, 7")
    metric_result["precision for k = 3"].append(calculateAvgPrecision(result_df, 3))
    metric_result["reciprocal rank for k = 3"].append(calculateReciprocalRank(result_df, 3))
    
    metric_result["precision for k = 5"].append(calculateAvgPrecision(result_df, 5))
    metric_result["reciprocal rank for k = 5"].append(calculateReciprocalRank(result_df, 5))
    
    metric_result["precision for k = 7"].append(calculateAvgPrecision(result_df, 7))
    metric_result["reciprocal rank for k = 7"].append(calculateReciprocalRank(result_df, 7))
    ...
    metric_result_df = pd.DataFrame(data=metric_result)
    print(metric_result_df)
    metric_result_df.to_csv(destination + "\\" + "CBIR metric.csv")

    del siamese_model
    return result_df, metric_result_df, MAP, MRR
...
if __name__ == "__main__":
    ...
    # dataset_path = "J:\\OCT retrieval\\Dataset\\Duke-AMD-DME - Copy\\"
    # query_img_path = "J:\\OCT retrieval\\Dataset\\Duke-AMD-DME - Copy\\AMD1\\"
    # np_seed = 2
    # tf_seed = 2
    ...
    tf_seed = int(sys.argv[4])
    
    dataset_name = dataset_path.split("\\")[-2]
    print(query_img_path, dataset_name, dataset_path, np_seed, tf_seed)

    np.random.seed(np_seed)
    # setting seed for tensoflow module
    set_seed(tf_seed)

    result = driver_single_image(f"{query_img_path}", f"{dataset_path}", f".\\result\\{dataset_name}\\seamese_net_avg_images_seed_np_{np_seed}_tf_{tf_seed}_for_k_3_5_7\\", dataset_name, np_seed, tf_seed)
    # print(f"MAP : {MAP} and MRR {MRR}")
